...
public class YcAnnotationConfigApplicationContext implements YcApplicationContext {
        ...
        try {
            ...
            for (Class cls : configClasses) {
                ...
                if (cls.isAnnotationPresent(YcComponentScan.class)) {
                    //如果有 则说明此配置类上有 @YcComponentScan 则 读取 basePackage
                    ...
                    logger.info(cls.getName() + "类上有@YcComponentScan注解,他要扫描的路径为:" + basePackages[0]);
                }
                //将这些包中的带有IOC注解的类 加载到一个 Map 中
                //开始扫描这些basePackages 包下的bean 并加载包装成 BeanDefinition 对象 存到 beanDefinitionMap
                recursiveLoadBeanDefinition(basePackages);
            }
            //循环 beanDefinitionMap  创建bean(是否为懒加载 是单例) 存到beanMap
            createBean();
            //循环所有托管的beanMap 中的 bean， 看属性和方法上是否有 @Autowired @Resource @Value 考虑DI
            doDi();
            System.out.println();
    ...
    private void doDi() throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        //循环的是  beanMap 这是托管 Bean
        for (Map.Entry<String, Object> entry : beanMap.entrySet()) {
            String beanId = entry.getKey();
            Object beanObj = entry.getValue();
            //TODO:情况一 方法上有 @YcResource 注解的情况
            //TODO:情况二 构造方法上有 @YcResource 注解的情况
            ...
            for (Field field : fields) {
                if (field.isAnnotationPresent(YcResource.class)) {
                    ...
                    //从 beanMap 中找 是否 singleton 是否 lazy
                    Object obj = getToDiBean(toDiBeanID);
                    //注入
                    field.setAccessible(true); //因为属性是private 所以要将它 accessible 设为 true
    ...
    private Object getToDiBean(String toDiBeanID) throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        ...
        //判断 beanMap 中没有此 bean 是否因为 lazy
        ...
        if (bd.getScope().equalsIgnoreCase("prototype")) {
            ...
            String classpath = bd.getClassInfo();
            Object beanObj = Class.forName(classpath).newInstance();
            //     beanMap.put( toBeanId, beanObj); 原型模式下 每次getBean 创建一次 bean 所以 beanMap不存
            return beanObj;
        }
        return null;
    }
...
//        });
        ...
        for (File cf : classFiles) {
            if (cf.isDirectory()) {
                ...
                //是 class 文件 则 取出文件 判断此文件对应的class 中是否有 @Component 注解
                ...
                if (cls.isAnnotationPresent(YcComponent.class)
                    ...
                    //扫描到YcLazy 把Lazy属性的值设为true
                    if (cls.isAnnotationPresent(YcLazy.class)) {
                        bd.setLazy(true);
                    }
                    //扫描到YcScope 把值传给scope属性
                    ...
                    //classInfo 中 保存这个待托管的类的包路径 com.yc.dao.UserDaoImpl
                    //cls.newInstance()
                    bd.setClassInfo(basePackage + "." + cf.getName().replaceAll(".class", ""));
                    //存到 beanDefinnitionMap 中  "beanId" -> "BeanDefinition对象"
     ...
     * 2.@Component("beanId")
     * <p>
     * YcConfiguration注解用类的全路径做beanid
     *
     * @param cls
     * @return
     */
    ...
    public Object getBean(String beanid) {
        ...
        if (scope.equalsIgnoreCase("prototype")) {
            //原型模式 每次 getBean 时 创建
            Object obj = null;
            try {
                Class.forName(bd.getClassInfo()).newInstance();
                //这种原型模式创建的bean 不能保存到 beanMap 中
                return obj;
            } catch (InstantiationException e) {
                e.printStackTrace();
            } catch (IllegalAccessException e) {
